...
we can get that literals in $F(\Phi,\lambda)$ have high probabilities to be backbone literals. However, the set $F(\Phi,\lambda)$ also contains non-backbone literals. We propose a heuristic strategy to remove non-backbone literals from $F(\Phi,\lambda)$.
...
%
A \emph{strongly connected component} (SCC) $s$ of $G_\Phi$ is subgraph of $G_\Phi$ such that there is a path between each nodes.
We use $G_\Phi^{scc}$ to denote the set of SCCs of $G_\Phi$. 
Let $\var_{scc}(\Phi)$ (resp. $\Lit_{scc}(\Phi)$, $E_{scc}(\Phi)$) denote the set of variables (resp. literals and edges) appeared in $G_\Phi^{scc}$,
and $\Phi_{scc}$ denote the set of clauses labeled to the edges in $G_\Phi^{scc}$.
...
Given a formula $\Phi$, the formula $\Phi_{scc}$ is satisfiable. Moveover, for every model $\lambda$ of $\Phi_{scc}$, 
the assignment $\lambda[\neg \var_{scc}(\Phi)]$ is a model $\Phi_\pi$.
...
\begin{proof}
Consider the assignment $\lambda$ of $\Phi_{scc}$ such that for each pair of edges $(l_1,\phi,l_2),(l_1',\phi,l_2')\in E_{scc}(\Phi)$,
$\lambda\models l_1$ iff $\lambda\models l_1'$. If $\lambda\models l_1$ holds for every edge $(l_1,\phi,l_2)\in E_{scc}(\Phi)$,
then, $\lambda$ is a model of  $\Phi_{scc}$. On the other hand, if $\lambda\models l_1$ does not hold (i.e., $\lambda\models \neg l_1$ holds)
then  $\lambda\models l_2'$ holds for every edge $(l_1',\phi,l_2')\in E_{scc}(\Phi)$. The results immediately follows.
\end{proof}
%A \emph{mutating set} $M(\pi)$ of a path $\pi$, is a set of literals in $\Lit(\pi)$ occurred at even positions, i.e., $M(\pi)=\{l_{2k}\mid 1\leq k\leq %\frac{|\pi|}{2}\}$. For example, given a path $\pi=l_1l_2...l_{2k}$, $M(\pi)=\{l_2, ..., l_{2k}\}$.


\begin{lemma}
Given a satisfiable formula $\Phi$, if $\var_{scc}(\Phi)\cap \var(\Phi\setminus\Phi_{scc})=\emptyset$ or $\Lit_{scc}(\Phi)\cap \Lit(\Phi\setminus\Phi_{scc})=\{l\}$, where the literal $l$ is a non-backbone literal for the formula $\Phi\setminus\Phi_{scc}$, then the literals in $\Lit_{scc}(\Phi)$ are non-backbone literals.
%$l$ is a non-backbone literal if for every $l'$ such that $l,l'\in\phi$ for some $\phi\in\Phi$, either $l'$ is a non-backbone literal or there exists a valid %mutable path $\pi\in\Pi(ll')$ in $G_\Phi$ such that the formula $\Phi\wedge\bigwedge_{l''\in M(\pi)} l''$ is satisfiable.

%Given a satisfiable formula $\Phi$, for every valid mutable path $\pi$ in $G_\Phi$, %$\in \Pi(l)$, 
%if $\var(\pi)\cap \var(\Phi\setminus\Phi_\pi)=\emptyset$ or $\var(\pi)\cap \var(\Phi\setminus\Phi_\pi)=\{x\}$, where the literals $x$ and $\neg x$ are %non-backbone literals for the formula $\Phi\setminus\Phi_\pi$, then the literals in $\Lit(\pi)$ are non-backbone literals.
%$l$ is a non-backbone literal if for every $l'$ such that $l,l'\in\phi$ for some $\phi\in\Phi$, either $l'$ is a non-backbone literal or there exists a valid %mutable path $\pi\in\Pi(ll')$ in $G_\Phi$ such that the formula $\Phi\wedge\bigwedge_{l''\in M(\pi)} l''$ is satisfiable.
\end{lemma}

\begin{proof}
%Suppose $\pi=l_1...l_n$ is a valid mutable path $\pi$ in $G_\Phi$.
If $\var_{scc}(\Phi)\cap \var(\Phi\setminus\Phi_{scc})=\emptyset$, then for every $x\in\var_{scc}(\Phi)$, $\Phi\setminus\Phi_{scc}$ does not contain any
literal of the form $x$ or $\neg x$. Let $\lambda$ be a model of $\Phi$.
By Proposition \ref{prop:scc-path}, $\lambda[\neg\var_{scc}(\Phi) ]\models\Phi_{scc}$.
Therefore, $\lambda[\neg\var_{scc}(\Phi) ]\models\Phi\setminus\Phi_{scc}$.
The result immediately follows. 

If $\Lit_{scc}(\Phi)\cap \Lit(\Phi\setminus\Phi_{scc})=\{l\}$, where the literal $l$ is a non-backbone literal for the formula $\Phi\setminus\Phi_{scc}$,
...
for every $y\in\var_{scc}(\Phi)$, $\lambda_i'(y)=i$, and for every $y'\in \var(\Phi)\setminus\var_{scc}(\Phi)$,
...
the literals in $\Lit_{scc}(\Phi)$ are non-backbone literals.
