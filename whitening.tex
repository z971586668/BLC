...
\begin{itemize}
\item there exists a clause $\phi\in\Phi$ with $l_1,l_2\in \phi$, labeled with $\phi$
\item  $l_1=\neg l_2$, without labels
\item $l_1=l_2\in\Phi$ (i.e., $l_1$ is a clause in $\Phi$), with out labels.
\end{itemize}
...
the literals  $l_1,l_2,...,l_n$. A path $l_1, l_2$ is \emph{trivial} if $l_1=\neg l_2$ or there exists a clause $\phi$ such that $l_1,l_2\in\phi$.


 For a path, the literals on the path is refereed as the literals on the path is refereed as $\Lit(\pi)$. A valid path $l_1l_2l_3...l_{n-2}l_{n-1}l_n$ is \emph{mutate} if $l_2=\neg l_3...l_{2k}=\neg l_{2k+1}, 1<k<\frac{2}{n}-1$. Suppose every literal in $\Lit(\pi)$ just appears in the set of label clauses on this path, refereed $\Phi_{\pi}$. The assignment generated by complementing every literal in $\Lit(\pi)$ will satisfy every clause in $\Phi_{\pi}$. The set of valid paths that started from literal $l$ is refereed as $\Pi(l)$. For every path in $\Pi(l)$, if $\Lit(\Pi(l))\setminus\{l\}$ is non-backbone literals and a new model formed when complementing every literal on the path simultaneously, then $l$ is a non-backbone literal.
