...
The benchmark consists of two parts: 388 industrial formulae and 6606 random formulae. 
Industrial formulae were selected from industrial verification which were selected from SAT competitions between 2002 and 2015.
...
On the random formulae side, we created 255 unsatisfiable random formulae using LBX \cite{MPA2015}, from which
we extracted 6606 satisfiable formulae using the following approach.

We first introduce some notations.
For each unsatisfiable formula $\Phi$, a subset of clauses $\Psi\subset \Phi$ is called maximal satisfiable subset  of $\Phi$
...
For a given unsatisfiable formula $\Phi$, there may exist more than one maximal satisfiable subsets of $\Phi$. 
We use LBX \cite{MPA2015} to generate 6606 maximal satisfiable subsets from the 255 unsatisfiable random formulae. 
...
Table \ref{tab:ind} shows the overview of experimental results on industrial formulas. 
...
The \textcolor{red}{red} line represents for the time used by the SAT solver for computing the first model, which indicates the hardness of the formula.
Lines with crosses represents for the running time of \tool, and lines with boxes represents for the running time of cb100. 
For most of hard formulae, \tool outperforms cb100 in terms of total running time and  average time of each SAT solver call. This demonstrates that \tool is more feasible for computing backbone of hard formulae. In particular, \tool runs twice time faster than cb100 on the formulae from the \emph{manthey} family. 
...
In this section, we present experimental results on random formulae.  
...
\label{tab:mcs-graph}
\end{table*}
Figure \ref{fig:mcs-time} depicts the running time of results on randomly selected 100 random formulae whose time of computing the first model range from 1s to 5s. This result demonstrates that there is no relation between the running time of backbone computing and running time of computing the first model, which is different from the result on industrial formulae. The reason is that random formulae have more complex community structures \cite{NZG2014,LJG2015SAT,LJG015}. Therefore, we cannot use running time of computing the first model to measure how hard a formula is.
