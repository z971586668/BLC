...
During each iteration, we first compute a ordered list $C$ of literals based on heuristic searching strategies, then select one literal  by one literal from 
...
assign $\lambda[\neg l]$ to $\lambda$ which will be severed as the model of $\Phi$ at step).
...
Instead, we manage to generate new models by changing values of literals in the known model. 
We use the following four heuristic searching strategies to select literals for generating new models.
