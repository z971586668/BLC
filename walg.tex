...
\begin{enumerate}
    \item ${\sf HS}_1(\Phi$): sort $\Lit(\Phi)$ in increase order of literals frequency. The result of the sort is a list of all literals in $\Lit(\Phi)$ with an increasing order of frequency.  We mark every literal in $\Lit(\Phi)$ with an identical ID such as $l_i$. Given two literals $l_i$, $l_j$, $i < j$ iff $\f(i)<\f(j)$. ${\sf HS}_1$ is a set consists of all the marked literals. The result of ${\sf HS}_1$ is a partial order $\{l_i, \prec\}$ such that for every $l_i$, $l_j$, $i<j$ iff $\f(l_i)<\f(l_j)$.
    \item ${\sf HS}_2(\Phi$): sort $\Lit(\Phi)$ in increase order of literals density. The result of the sort is a list of all literals in $\Lit(\Phi)$ literals with an increasing order of density.
 \end{enumerate}
 ...
 If the frequency is low, then it means that less clauses might be affected by the changing assignment of the literal. Therefore, the assignment generated by negating the literal is more likely to be a model, the literal is more likely to be a free literal of the given formula to the given model.

 The density of literals consider the clauses length. With a longer clauses, there is a higher possibility that the clauses can be satisfied by another literal. The longer the clauses are, the miner the density of literal is. Therefore, literals with low density are more likely to be free literals.
...
\section{Computing $\BLap(\Phi)$}
 ...
 We search for the pathes that starting from a literal $l$, ending at $\neg l$. If every literal at the $2k_{th}$ position is the negation of the literal at $2k+1_{th}$ position, expect for the starting and ending literal, k starts from 1, then we possibly can generate a new model by negating the assignments of every literals on the path. However, if no such paths found, no models can be generated by negating the assignment of the given literal.
...
\end{algorithm}
 ...
 In Line 1, Whitening Algorithms is applied with a given formula $\Phi$ and the under-approximation of non-backbone obtained in Algorithm \ref{alg:wal}. From Line 2 to Line 12, a heuristic searching approach is applied to search paths in the community structure graph. $\Phi^k_x$ represents the $k_{th}$ literal in a path starting from literal x, which means that after go through k clauses, we are able to reach literals in $\Phi^k_x$. Only clauses that contain the negation of current literal on the path will be explored to reduce the searching space as shown in Line 6.
