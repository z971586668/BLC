...
$l$ is a backbone literal or not by checking the satisfiability of $\Phi\wedge l$.
In general, one can direct applying this test to compute backbone without the two approximations. However, 
the test heavily replies on SAT solver which may be time-cost. 
Intuitively, literals in $\BLap(\Phi)\setminus \NBLap(\Phi)$ have a high probability to be backbone literals.
...
After this step, the exact backbone and non-backbone are found.  As shown in the right part of Figure \ref{flow}, 
our approach reduces the number of SAT solver calls, as we only do test to literals in the dashed area. 
